    ...
    if let Some(script_name) = tokenizer.next() {
        if let Some(command) = tokenizer.next() {
            match command {
                ...
                "detach-pre" => parse_params(player, tokenizer, Command::DetachPre, script_name),
                _ => Err(
                    "Enter a valid subcommand: attach, attach-pre, detach, or detach-pre."
                        .to_string(),
                ),
            }
        } else {
            Err("Enter a subcommand: attach, attach-pre, detach, or detach-pre.".to_string())
        }
    } else {
...
impl Command {
    fn into_action(self, entity: Entity, script_name: &str, id: Either<Id, String>) -> ActionEvent {
        ...
        match self {
            ...
            .into(),
            Command::DetachPre => ScriptDetach {
                entity,
                script,
                pre: true,
                target: id,
